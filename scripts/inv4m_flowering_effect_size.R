...
# non-inverted sisters within the same family.  Runs at TWO grouping levels:
...
#   (a) group shared-control (all inv4m vs all non-inverted sisters)
#   (b) paired per-family (inv4m mean vs non-inverted sister mean, paired by family)
#   (c) per-family forest-style plot of effect sizes with CIs
#
# Effect sign convention: mean(inv4m) - mean(non-inverted sister)
#   negative value => inv4m carriers flower earlier than their non-inverted sisters.
# ==============================================================================
...
INV4M_LABEL  <- "inv4m"
SISTER_LABEL <- "non-inverted sister"

# purple used for the inv4m dots in the forest plot
INV4M_COLOR  <- "#551a8b"
...
bootstrap_diff <- function(sis_vals, teo_vals, n_boot = 5000) {
  # effect size: inv4m - non-inverted sister  (matches dabestr with idx = c("non-inverted sister","inv4m"))
  ...
  obs <- mean(teo_vals) - mean(sis_vals)
  boots <- replicate(n_boot, {
    mean(boot_sample(teo_vals)) - mean(boot_sample(sis_vals))
  })
...
run_pipeline <- function(family_tag, n_chars, family_label) {
  ...
  df <- corr |>
    ...
    filter(allele %in% c(SISTER_LABEL, INV4M_LABEL))
  ...
  cat(sprintf("Usable %s families (>=1 inv4m and >=1 non-inverted sister line): %d\n",
  ...
  df_family <- df_geno |>
    ...
    group_by(family) |>
    filter(all(c(SISTER_LABEL, INV4M_LABEL) %in% allele)) |>
    ungroup()
  ...
  run_group <- function(data, trait, level_tag) {
    ...
    db <- dabestr::load(
      ...
      idx  = c(SISTER_LABEL, INV4M_LABEL)
    )
    ...
    save_db_plot(
      ...
      subtitle = paste0("All inv4m lines vs all non-inverted sisters (",
  ...
  run_paired_family <- function(trait) {
    ...
    save_db_plot(
      ...
      subtitle = paste0(family_label,
                        ": each family = one pair (inv4m mean vs non-inverted sister mean)"),
      width = 6, height = 5
    )
    eff
  }
  ...
  forest_plot <- function(data, trait, level_tag) {
    ...
    p <- ggplot(eff_df, aes(x = point, y = fam_label)) +
      ...
      labs(
        x     = paste0("Mean difference (inv4m \u2212 non-inverted sister), ",
                       TRAIT_LABELS[[trait]]),
        y     = paste0(family_label, " (n per arm)"),
        title = paste0("Per-family effect size: ", TRAIT_LABELS[[trait]],
                       " (", family_label, ", ", level_tag, "-level)"),
        subtitle = "Negative = inv4m flowers earlier; 95% bootstrap CI"
      ) +
